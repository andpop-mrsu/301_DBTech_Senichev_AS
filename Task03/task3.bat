...
echo --------------------------------------------------
sqlite3 movies_rating.db -box "SELECT id, name, register_date FROM users WHERE SUBSTR(name, INSTR(name, ' ') + 1) LIKE 'A%' ORDER BY register_date LIMIT 5;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box "SELECT u.name, m.title, m.year, r.rating, date(r.timestamp, 'unixepoch') as rating_date FROM ratings r JOIN users u ON r.user_id = u.id JOIN movies m ON r.movie_id = m.id ORDER BY u.name, m.title, r.rating LIMIT 50;"
echo " "
...
sqlite3 movies_rating.db -box "SELECT occupation, COUNT(*) as count FROM users GROUP BY occupation ORDER BY count DESC LIMIT 1;"
echo " "

echo "Самая редкая профессия:"
sqlite3 movies_rating.db -box "SELECT occupation, COUNT(*) as count FROM users GROUP BY occupation ORDER BY count ASC LIMIT 1;"
echo " "